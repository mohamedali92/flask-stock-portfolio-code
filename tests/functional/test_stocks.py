...
def test_get_add_stock_page_not_logged_in(test_client):
    ...
    assert b'Stock Symbol <em>(required)</em>' not in response.data
    assert b'Number of Shares <em>(required)</em>' not in response.data
    assert b'Purchase Price ($) <em>(required)</em>' not in response.data
...
def test_post_add_stock_page(test_client, log_in_default_user, mock_requests_get_success_daily):
    """
    GIVEN a Flask application
    WHEN the '/add_stock' page is posted to (POST) for a logged in user
    ...
    assert b'Added new stock (AAPL)!' in response.data  # NEW!


def test_get_list_stocks_not_logged_in(test_client):
    """
    GIVEN a Flask application
    WHEN the '/list_stocks' page requested (GET) by a user that's not logged in
    THEN check that the user is redirected to the '/user_login' page
    """
...
def test_get_stock_list_logged_in(test_client, add_stocks_for_default_user, mock_requests_get_success_daily):
    ...
    headers = [b'Stock Symbol', b'Number of Shares', b'Share Price', b'Purchase Date', b'Current Share Price',
               b'Stock Position Value', b'TOTAL VALUE']  # UPDATED!
    data = [b'SAM', b'27', b'301.23', b'2020-07-01',
            b'COST', b'76', b'14.67', b'2019-05-26',
            b'TWTR', b'146', b'34.56', b'2020-02-03']
    ...
    response = test_client.get('/stocks', follow_redirects=True)
    assert response.status_code == 200
    assert b'List of Stocks' in response.data   # Updated!!
    for header in headers:
        assert header in response.data
    for element in data:
        assert element in response.data
